...
3-column layout (col_left, col_center, col_right) replaces the old 2-column layout — the ECC controls + search bar now sit in the left column beside the graph.

Radio buttons are vertical (horizontal=False) with a scrollable list (max-height 370px) so they fit neatly in the narrow left panel.

Top-K and Pipeline moved to the right column, keeping them accessible but separate from the control picker.

Graph viewer occupies the wider center column for maximum visibility.

CSS for .ctrl-radio was updated from horizontal scrolling (overflow-x) to vertical scrolling (overflow-y, flex-direction: column).

13 minutes ago
make the export under the circles
...
Here's the updated code with the export button moved to below the center column (circles), using st.columns to place it under the graph:
...
if os.path.exists(DATA_FILE):
    ...
    with col_center:
        ...
        # ── Export — directly under the graph, inside center column ──
        pdf_bytes = generate_pdf(selected_id, src_text, mappings)
