    ...
    def compute_sparse_similarity(self, query_sparse: Dict[str, float], target_sparse: Dict[str, float]) -> float:
        ...
        for token, weight in query_sparse.items():
            if token in target_sparse:
                score += weight * target_sparse[token]
        return score
...
def create_mapping_dataframe_batched(
    ...
    dense_scores_matrix = np.dot(source_embeddings.dense, target_embeddings.dense.T)
...
def main():
    ...
    try:
        source_df = pd.read_csv("data/source_with_metadata_gpt_oss.csv")
        target_df = pd.read_csv("data/target_with_metadata_gpt_oss.csv")
